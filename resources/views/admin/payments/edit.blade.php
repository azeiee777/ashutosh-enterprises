...
<div class="admin-card"><div class="card-body">
    <form method="POST" action="{{ route('admin.payments.update', $payment) }}" class="admin-form" enctype="multipart/form-data" x-data="paymentForm()">@csrf @method('PUT')
        <div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">Amount (₹) (Optional)</label><input type="number" step="0.01" name="amount" class="form-control" value="{{ old('amount', $payment->amount) }}">@error('amount')<small class="text-danger">{{ $message }}</small>@enderror</div>
            <div class="col-md-4">
