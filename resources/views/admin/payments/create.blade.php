...
@section('breadcrumb', 'Payments / Add New')

@section('content')
<div class="page-title-section"><div><h1>Add Payment Record</h1></div><a href="{{ route('admin.payments.index') }}" class="btn btn-admin-secondary"><i class="bi bi-arrow-left"></i> Back</a></div>

<div class="admin-card"><div class="card-body">
    <form method="POST" action="{{ route('admin.payments.store') }}" class="admin-form" enctype="multipart/form-data" x-data="paymentForm()">@csrf
        <div class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label">Reference Number (Cheque/Txn ID)</label><input type="text" name="reference_number" class="form-control" value="{{ old('reference_number') }}"></div>
            <div class="col-md-6"><label class="form-label">Paid To / Received By</label><input type="text" name="paid_to" class="form-control" value="{{ old('paid_to') }}"></div>
            ...
            <div class="col-12"><label class="form-label">Attachment (PDF/Image)</label><input type="file" name="attachment" class="form-control" accept=".pdf,.jpg,.jpeg,.png">@error('attachment')<small class="text-danger">{{ $message }}</small>@enderror</div>
            
            <div class="col-12"><button type="submit" class="btn btn-admin-primary"><i class="bi bi-check-lg"></i> Save Payment</button></div>
        </div>
    </form>
</div></div>
